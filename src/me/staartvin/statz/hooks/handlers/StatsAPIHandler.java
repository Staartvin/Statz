...
public class StatsAPIHandler implements DependencyHandler {
     ...
     * @param damageValue
     *            Damage value to check for. (negative number to not skip check)
     * @param worldName
     *            World to check in. Null for global.
     * @param statName
     *            Either "Block break" or "Block place"
    ...
    public int getBlocksStat(final UUID uuid, final int id, final int damageValue, final String worldName,
         ...
         * 
         * 
         * if (damageValue > 0) { checkDamageValue = true; }
         * 
         * for (final Object[] vars : blockStat.getAllVariables()) {
         * 
         * if (checkDamageValue) { // VAR 0 = blockID INT, VAR 1 = damageValue
         * BYTE, VAR 2 = (true = break, false = place) BOOLEAN
         * 
         * final byte byteValue = (Byte) vars[1];
         * 
         * if ((Integer) vars[0] == id && byteValue == damageValue) { value +=
         * blockStat.getValue(vars); } } else { if ((Integer) vars[0] == id) {
    ...
    public int getNormalStat(final UUID uuid, final String statName, final String worldName) {
        ...
        /*
         * 
         * for (final Object[] vars : stat.getAllVariables()) { value +=
         * stat.getValue(vars); }
         * 
         * return value;
         */
    ...
    public StatsHolder getStatsHolder(UUID uuid) {
        ...
        // Users are only loaded when they are offline, so try to force-load
        // them.
        if (user == null) {
            try {
