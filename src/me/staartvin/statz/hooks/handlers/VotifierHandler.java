...
public class VotifierHandler implements DependencyHandler {
	...
	public boolean setup(final boolean verbose) {
		if (!isInstalled()) {
			...
			if (api != null) {
				if (verbose) {
					plugin.getLogger().info(Dependency.VOTIFIER.getInternalString() + " has been found and can be used!");
				}
				return true;
			} else {
				if (verbose) {
					plugin.getLogger().info(Dependency.VOTIFIER.getInternalString() + " has been found but cannot be used!");
				}
				return false;
			}
		}
